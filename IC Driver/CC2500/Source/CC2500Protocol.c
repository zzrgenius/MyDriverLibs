...
//西马遥控器相关
#define XIMA_RC_CMD_CODE		0xf0//西马遥控命令码
...
*****************************************************************************/
uchar f_CC2500_GetSendBuffer(uchar * pBuf, uint len)
{
...
/*****************************************************************************
 函 数 名: f_CC2500_RevProtocolDeal
 功能描述  : 接收到数据包后处理
 输入参数: pData数据缓存
		   len缓存长度
 返 回 值: 
...
*****************************************************************************/
uchar f_CC2500_RevProtocolDeal(uchar * pData, uchar len)
{
	static uchar frameIndex = 100;					//遥控器从100开始,防止一致
		...
		else if (XIMA_RC_CMD_CODE == pData[2]) //正常模式
		{
			if (pData[XIMA_FRAME_LEN - 1] == f_CalcAddSum(pData, XIMA_FRAME_LEN - 1))
			{
				...
				if (frameIndex != pData[FRAME_INDEX_POS])
				{
					...
					//比较当前ID(保留ID直接校验通过)
					if (f_IsRevMachineID(&pData[MACHINE_CODE_POS], MACHINE_CODE_LEN) || f_CompareValue(machineID, &pData[MACHINE_CODE_POS], MACHINE_CODE_LEN))
					{
						//红外发送数据启动
						f_IR_StartSend(&pData[COMM_HEAD_LEN], PACKET_LEN);
					}
