...
#ifdef  VAR_CC2500_GLOBALS
	#define  VAR_CC2500_EXT
#else
	#define  VAR_CC2500_EXT  extern
#endif

#define CC_SOMI_IN					PBI_4
#define CC_GDO0_IN           		PBI_5

#define CC_UCLK_HIGH			(PDO_4 = 1)
#define CC_UCLK_LOW				(PDO_4 = 0)
#define CC_SIMO_HIGH			(PAO_3 = 1)
#define CC_SIMO_LOW				(PAO_3 = 0)

#define CC_CS_HIGH       			(PAO_1 = 1)	        
#define CC_CS_LOW              		(PAO_1 = 0)
...
{
	uchar RF_Channel[4];
	uchar RF_Channel_Index;
	uchar nCC2500ErrFlag : 1;//初始化报错
	uchar nCC2500Time1;	// 2S进RX模式
	uint nCC2500Time2;	//5分钟读取一次寄存器
}CC2500_INF;
VAR_CC2500_EXT AT_NEAR CC2500_INF Cc2500Inf;

VAR_CC2500_EXT uchar f_SendPacket(uchar *txBuffer, uchar size);
VAR_CC2500_EXT uchar halRfReceivePacket(uchar *rxBuffer, uchar *length);
VAR_CC2500_EXT uchar f_InitCC2500(uchar Channel);
VAR_CC2500_EXT void f_Cc2500Init(void);
VAR_CC2500_EXT void f_RxCC2500(void);
VAR_CC2500_EXT void SetCC2500_ChannelNumber(uchar number);
#endif	//__CC2500_H