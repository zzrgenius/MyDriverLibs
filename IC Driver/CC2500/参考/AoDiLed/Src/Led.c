	...
	else if(pFuctionData->bInitInf == rSYS_INIT_OK)
	{
		...
		else
		{
			...
			else if(pFuctionData->SystemCurState == RUNNING_STATE)
			{
				...
				{
					//电源灯
					if(pFuctionData->systemErrInf.flag.bZeroCrossErr == false)
					{
						LedPowerIcon = true;
					}
					else
					...
					//水温
					if(pFuctionData->systemErrInf.flag.bHeatCommunErr == false)
					{
...
//								LedWaterGFlickF = true;
								LedWaterRFlickF = false;
								LedWaterBFlickF = true;
								LedWaterGFlickF = false;
							}
						}
...
//							LedWaterGFlickF = true;
					...
					else if(pFuctionData->systemErrInf.flag.bDryTempHighErr)
					{
						LedDryRFlickF = true;
						LedDryGIcon = false;
						LedDryBIcon = false;
...
void f_pLedDriveDeal(void)
{
	...
	LED_TEMP_DRY_BULE(1);
	LED_TEMP_DRY_GREEN(1);
	if(LedPowerIcon || (LedPowerFlickF && LedPowerFlickS)||(LedPower500msf && LedPower500mss))
	{
		LED_POWER(0);
	}
