...
void main(void)
{
	...
	while (1)
	{
		...
		f_RfRecDeal();
		f_Uart1SendDeal(&machineInf);
		f_Uart1ReceDeal(&machineInf);
		if(t1msFlag)
		{
			t1msFlag = 0;
		...
		if(t10msFlag)
		{
			...
			{
				f_LedDeal(&machineInf);
				f_LedDriveDeal();//LED底层驱动
				if(testPL > 0)
				{
					testPL--;
				}
			...
			{
				f_WriteOneDataEeprom();	//EEPROM写入数据
			}
			tTime.t100msTask++;
