 ...
 *----------------------------------------------*/
//独立处理按键
KEY_DRIVER_INDE_KEY keyDriverIndeKey[KEY_DRIVER_INDE_KEY_NUM + 1];
...
void f_KeyDriverDeal(KEY_DRIVER       *pKeyDriver)
{
	...
	//按键掩码处理，防止卡按键影响
	if (KEY_DRIVER_NO_KEY != pKeyDriver->data.getKeys)
	{
		pKeyDriver->data.newName = pKeyDriver->data.getKeys & pKeyDriver->data.mask;
	}
	else//按键一旦全部释放,则认为全部正常
	{
		pKeyDriver->data.mask = KEY_DRIVER_ALL_KEY;
		pKeyDriver->data.newName = pKeyDriver->data.getKeys;
	}
